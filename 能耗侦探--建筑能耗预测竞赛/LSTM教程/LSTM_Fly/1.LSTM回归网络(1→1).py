...
# test的最后一行是NAN
testY_ = testY[ :  , : -1]
testPredict_ = testPredict[ :-1 , : ]
testScore = math.sqrt(mean_squared_error(testY_[0], testPredict_[:,0]))
print('Test Score: %.2f RMSE' % (testScore))

# 计算准确率
# print(testPredict.shape) # (47,1)
# print(testY.shape)  # (1 , 47)

# 最后一个数是 NAN
accuracy = numpy.mean(1 - (numpy.abs(testPredict_ - testY_.T)/testY_.T))
print('测试集准确率 : ' , accuracy)  # epochs50 : 90.3% , epochs100 : 90.2% , epochs300 : 89.8%
plt.plot(testPredict[ : 30 , : ])
plt.plot(testY.T[ : 30 , : ])
plt.show()
